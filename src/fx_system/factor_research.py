...
from .factors import build_factor_panel, factor_catalog, factor_columns
...
def screen_factors(train: pd.DataFrame, settings: FactorSettings) -> tuple[list[str], pd.DataFrame]:
    ...
    for feature in features:
        coverage = float(clean[feature].notna().mean())
        valid = pd.DataFrame({"value": clean[feature], "label": train["_label"]}).dropna()
        information_coefficient = (
            float(valid["value"].corr(valid["label"], method="spearman"))
            if len(valid) >= 50 and valid["value"].nunique() > 2
            else 0.0
        )
        ...
        rows.append(
            {
                ...
                "quintile_target_spread": _bucket_spread(clean[feature], train["_label"]),
            }
        )
    statistics = pd.DataFrame(rows).sort_values("absolute_ic", ascending=False)
    ...
    if not eligible:
        raise RuntimeError("No factor passed coverage and variance checks")
...
def write_factor_artifacts(
    ...
    stability = _factor_stability(all_statistics, all_coefficients)
...
def _factor_stability(statistics: pd.DataFrame, coefficients: pd.DataFrame) -> pd.DataFrame:
    ...
    result = stats.join(coefficient_stats, how="outer").reset_index()
    return result.sort_values(["selected_folds", "mean_ic"], ascending=[False, False])
...
def _markdown_factor_report(
    ...
    factor_rows = "\n".join(
        f"| {row.factor} | {int(row.selected_folds)} | {row.mean_ic:.4f} | "
        f"{row.mean_coefficient:.4f} | {row.coefficient_sign_stability:.2%} |"
...
Probabilities are Platt-calibrated on a later, purged slice of the training window. Entry requires
positive expected R after pair-specific spread, slippage, commission, and an extra cost buffer.
...
{summary["positive_folds"]}/{summary["folds"]}; total trades: {summary["total_trades"]}.

{holdout_section}

## Most stable selected factors

| Factor | Selected folds | Mean IC | Mean standardized coefficient | Coefficient sign stability |
|---|---:|---:|---:|---:|
{factor_rows}
